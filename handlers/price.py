...
async def price(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await show_price_page(update.message, context, page, category_filter)

async def show_price_page(message, context, page, category=None):
    try:
        ...
        total = len(records)
        if total == 0:
            await message.reply_text("No products found.")
            return
        ...
        for row in items:
            ...
            if 'IMAGE("' in image:
                ...
                await message.reply_photo(photo=url, caption=text)
            else:
        ...
        if buttons:
            await message.reply_text("Page controls:", reply_markup=InlineKeyboardMarkup([buttons]))
    except Exception as e:
        await message.reply_text(f"❌ Error loading products: {e}")
